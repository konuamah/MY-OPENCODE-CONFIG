import { isCommentLine, detectCommentMarkers } from "./utils";
const MAGIC_EXCEPTIONS = /^(0|1|-1|0\.0|1\.0|100|100\.0)$/;
export function checkMagicValues(ctx) {
    const result = [];
    for (let i = 0; i < ctx.lines.length; i++) {
        ...
        const magicMatch = trimmed.match(/[=:]\s*(-?\d+(?:\.\d+)?)\s*(?:$|[+\-*/),;\]}])/);
        if (!magicMatch)
            continue;
        const value = magicMatch[1];
        if (MAGIC_EXCEPTIONS.test(value))
            continue;
        const lineNum = i + 1;
        result.push({
            rule: "G25",
...
export function checkTooManyParams(ctx) {
    const result = [];
...
export function checkSingleLetterNames(ctx) {
    const result = [];
    for (let i = 0; i < ctx.lines.length; i++) {
        const line = ctx.lines[i];
        const trimmed = line.trim();
        ...
        if (/for\s*\(?\s*[a-z]\s*(?:in|of|;)/.test(trimmed))
            continue;
        let match;
...
export function checkFlagArgs(ctx) {
    const result = [];
    ...
    for (let i = 0; i < ctx.lines.length; i++) {
        ...
        while ((match = callPattern.exec(trimmed)) !== null) {
            const call = match[0];
            if (/function|def|fn|fun|func/.test(call))
                continue;
...
export function checkCommentedOutCode(ctx) {
    const result = [];
...
export function checkDeepNesting(ctx) {
    const result = [];
    for (let i = 0; i < ctx.lines.length; i++) {
        const line = ctx.lines[i];
        const trimmed = line.trim();
...
export function checkDeadFunctions(ctx) {
    const result = [];
