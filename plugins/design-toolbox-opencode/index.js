...
function runDetector(filePath) {
    ...
    if (!existsSync(detectPath))
        return null;
    if (!filePath.match(UI_FILE_PATTERN))
        return null;
...
function runTasteChecks(content, filePath) {
    const issues = [];
    // Em-dash ban
    if (/—/g.test(content)) {
